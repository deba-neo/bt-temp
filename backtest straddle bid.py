...
if __name__ == "__main__":
    ...
    test_start_date = "01-04-2023"

    bid_threshs = [25,30,35,40,45,50,75,100,125,150]
    # bid_thresh=30
    ...
    for bid_thresh in bid_threshs:
        op_df=pd.read_parquet(f'strd_trades/Strd_bid_{bid_thresh}.parquet')

        # mask=op_df['Bid_Time']!=0
        # op_df=op_df.sort_values(by=['Date', 'Expiry']).drop_duplicates(subset=['Date'], keep='first')
        # op_df['Bid_Time']=op_df['Bid_Time'].dt.time.astype(str)
        # op_df['Date']=op_df['Date'].dt.strftime('%d-%m-%Y')
        # op_df['Expiry']=op_df['Expiry'].dt.strftime('%d-%m-%Y')
        # op_df.set_index('Trade_Date',inplace=True)

        excess_strat_args = {
            "sl_points": -2*bid_thresh,
            "fwd_hedge" : True,
        }

        for strat in strategies:
            sys.stdout = Logger(f"str-bid-out/nf-slpb-2023-{bid_thresh}.csv")
            Logger.always_output(sys.stdout, "Date, PnL, Buy Quantity, Sell Quantity, Buy Value, Sell Value, Total Lots\n")
            one_loop_ID("nif", strat, test_start_date, test_stop_date, new_init, starttime, endtime, weekly=True, excess_strat_args=excess_strat_args, datafile=op_df)
