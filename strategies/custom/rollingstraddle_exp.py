...
class RollingStraddle(ThetaGamma):
    def hedge(self, spot) -> None:
        ...
        if self.context.strategy_args.get("fwd_hedge", False):
            TG_STD3_2DTE_HedgebyForwards.hedge(self, spot)
            return
    ...
    def position_management(self, spot) -> None:
        ...
        if (
            ...
            or self.context.PnL() > self.context.strategy_args["profit_book"]
    ...
    def new_position_handler(self, spot) -> None:
        ...
        self.context.strategy_args["stop_loss"] = self.context.strategy_args["sl_points"]*self.context.demand
        self.context.strategy_args["profit_book"] = -self.context.strategy_args["sl_points"]*self.context.demand*0.5
        
        print(f"Stop Loss: {self.context.strategy_args['stop_loss']}")
        return
    
    def take_position(self, spot) -> None:
    ...
    def create_new_position(self, spot, hedge = False) -> None:
        ...
        std = self.context.atm_straddle_price(spot)[0]
        position1 = int(math.ceil((spot + std) / float(self.context.movement))) * self.context.movement
        position2 = int(math.ceil((spot - std) / float(self.context.movement))) * self.context.movement
        ...
        self.context.policy_variables["to_create_portfolio"] = {
            f"{position1}CE" : quantity_calls,
            f"{position2}PE" : quantity_calls,
        }
