...
class Delta_Skew(Strategy):
    ...
    def build_position(self, spot, delta_long):
        ...
        #     strike_buy = int(math.ceil((atm_strike + 0.5*atm_straddle_price)/strike_gap)*strike_gap)
        #     strike_sell = int(math.ceil((atm_strike + 1.5*atm_straddle_price)/strike_gap)*strike_gap)
        #     option_buy = f"{strike_buy}CE"
        #     option_sell = f"{strike_sell}CE"
        ...
        #     strike_buy = int(math.ceil((atm_strike - 0.5*atm_straddle_price)/strike_gap)*strike_gap)
        #     strike_sell = int(math.ceil((atm_strike - 1.5*atm_straddle_price)/strike_gap)*strike_gap)
        #     option_buy = f"{strike_buy}PE"
        #     option_sell = f"{strike_sell}PE"

        #     prices = self.context.get_current_price([option_buy, option_sell])
        #     rtv = prices[0] - prices[1]

        # self.context.strategy_args["stop_loss"] = -abs(1*rtv)*size
