...
class ExtraLong(TG_STD3_2DTE_HedgebyForwards):
    def position_management(self, spot) -> None:
        ...
        # required_level = math.ceil((t-t2).seconds/3600)
        ...
        #     self.build_position(spot, self.context.strategy_args["demand"])
    ...
    def build_position(self, spot, size):
        straddle_price, strike = self.context.atm_straddle_price(spot)

        call_position, put_position = self.context.find_strangle_strikes(strike, spot)
        
        wing1 = int(math.ceil((call_position+straddle_price*1.5) / float(self.context.movement))) * self.context.movement
        wing2 = int(math.floor((put_position-straddle_price*1.5) / float(self.context.movement))) * self.context.movement
        
        
        q = size

        self.context.policy_variables["to_create_portfolio"] = {
            f"{call_position}CE" : q,
            f"{put_position}PE" : q,
            f"{wing1}CE" : -1.4*q,
            f"{wing2}PE" : -1.4*q,
        }
